...
```{r}
#The log iris data on the first two discriminant axes.
...
```{r}
#The fgl data on the first two discriminant axes
...
#Define cushplot function
cushplot <- function(xp, yp, Z)
{
    plot(Cushings[, 1], Cushings[, 2], log = "xy", type = "n",main="",
...
```{r}
#Linear and quadratic discrirninant analysis applied to the Cushing's syndrome data.
par(mfrow = c(2,2))
...
```{r}
#Neural networks applied to the Cushing's syndrome data.
par(mfrow = c(2, 2))
pltnn("Size = 2")
...
```{r}
#Neural networks with three hidden units and lambda = 0.01 applied to the Cushing's syndrome data.
par(mfrow=c(1,2))
...
```{r}
#Calibration plot for multiple logistic fit to the fgl data.
